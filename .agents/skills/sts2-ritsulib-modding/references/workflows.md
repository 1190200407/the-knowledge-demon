...
1. `D:\My Projects\GodotProjects\the-knowledge-demon\.agents\RitsuLib-doc\README.md`
2. `D:\My Projects\GodotProjects\STS2-RitsuLib-0.4.33\docs`
3. 当前项目的 `*.csproj`
...
- `D:\My Projects\GodotProjects\STS2-RitsuLib-0.4.33` 是否存在且与项目引用版本匹配
- 运行时 `D:\SteamLibrary\steamapps\common\Slay the Spire 2\data_sts2_windows_x86_64\sts2.xml` 是否存在
...
2. `D:\My Projects\GodotProjects\STS2-RitsuLib-0.4.33\Interop\AutoRegistration\RegistrationAttributes.cs`
3. `D:\My Projects\GodotProjects\STS2-RitsuLib-0.4.33\Scaffolding\Content\ModCardTemplate.cs`
4. `D:\My Projects\GodotProjects\the-queen\Cards`
5. 当前仓库卡牌目录
...
3. 如果 exact public 接口名、override 面或 helper 参数不确定，先查 `STS2-RitsuLib-0.4.33` 源码。
...
2. `D:\My Projects\GodotProjects\STS2-RitsuLib-0.4.33` 中 `RegisterRelic`、`ModRelicTemplate`
3. 当前仓库遗物目录
4. `D:\My Projects\GodotProjects\the-queen\Relics`

默认执行顺序：

1. 确认 relic pool 与 rarity。
2. 如果 exact public 接口名、override 面或 helper 参数不确定，先查 `STS2-RitsuLib-0.4.33` 源码。
3. 确认模板基类。
...
2. `D:\My Projects\GodotProjects\STS2-RitsuLib-0.4.33` 中 Power 相关模板和注册器
3. 当前仓库 power 目录
4. `D:\My Projects\GodotProjects\the-queen\Powers`
...
- `powers.json`
- 数值显示和动态描述
- 如果 `Power` 的 public 接口名、hook 面或 helper 参数不清楚，先查 `STS2-RitsuLib-0.4.33` 源码
- 是否需要项目内统一 Power 基类
...
2. `D:\My Projects\GodotProjects\STS2-RitsuLib-0.4.33` 中 `RegisterPotion` 和 `ModPotionTemplate`
3. 当前仓库药水目录
...
3. `D:\My Projects\GodotProjects\STS2-RitsuLib-0.4.33\docs`
4. `D:\My Projects\GodotProjects\the-knowledge-demon\.agents\RitsuLib-doc\Migrations\02 - BaseLib 至 RitsuLib\README.md`
...
5. `STS2-RitsuLib-0.4.33` 与 `sts2.dll` / `sts2.xml` 是否都在，且和本地 `csproj` / 参考代码版本没有明显漂移
6. `csproj` 是否指向正确 DLL
7. 如果是 patch apply 失败，先对照 `STS2-RitsuLib-0.4.33` 源码与 `sts2.xml` 确认 target 签名、参数类型、参数名
8. 特性/模板/ID 是否写错
9. 本地化、图片、场景路径是否缺失
